class LamportNotesController < ApplicationController
  ...
  def index
    ...
    @lamport_notes_paginação = LamportNote.paginate(:page => params[:page], per_page: 3).where(user_id: current_user)
    if params[:search]
      @lamport_notes = LamportNote.search(params[:search]).order("created_at DESC").paginate(:page => params[:page], per_page: 3)
    else
      @lamport_notes = LamportNote.where(user_id: current_user).paginate(:page => params[:page], per_page: 3)
    end
